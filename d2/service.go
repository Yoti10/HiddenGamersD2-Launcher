...
type service struct {
	hiddengamersClient hiddengamers.Client
...
func (s *service) getAvailableMods() (*config.GameMods, error) {
	...
	contents, err := s.hiddengamersClient.GetAvailableMods()
	if err != nil {
		return nil, err
	}
...
func (s *service) applySlashPatch(path string, state chan PatchState, progress chan float32) error {
	state <- PatchState{Message: "Comprobando parche de hiddengamers..."}
	...
	if len(patchFiles) > 0 {
		state <- PatchState{Message: fmt.Sprintf("Actualizando %s al parche actual de hiddengamers", path)}
...
func (s *service) downloadFile(fileName string, remoteDir string, path string, counter *WriteCounter) error {
	...
	contents, err := s.hiddengamersClient.GetFile(f)
	if err != nil {
		return err
	}
...
func (s *service) getManifest(path string) (*Manifest, error) {
	contents, err := s.hiddengamersClient.GetFile(path)
	if err != nil {
		return nil, err
	}
...
func NewService(
	...
	s := &service{
		hiddengamersClient: hiddengamersClient,
